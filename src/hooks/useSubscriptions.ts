...
import { Subscription, IconKey, ICON_META } from '@/types/subscription'
...
export function useSubscriptions() {
  ...
  const add    = (s: Omit<Subscription, 'id'>) => save([...subs, { ...s, id: Date.now() }])
  const update = (s: Subscription)             => save(subs.map(x => x.id === s.id ? s : x))
